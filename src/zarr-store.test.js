...
describe('ZarrStore', () => {
  ...
  describe('v2', () => {
    it('loads metadata and coordinates arrays on initialization', async () => {
    ...
    it('reuses cached metadata', async () => {
      const source = 'https://example.com/v2-cached'
      const coordinateValues = [2000, 2001]
      ...
      const firstStore = new ZarrStore({
        source,
        version: 'v2',
        variable: 'temp',
        coordinateKeys: ['time'],
      ...
      await expect(firstStore.initialized()).resolves.toBe(true)
      expect(fetchMock).toHaveBeenCalledTimes(1)
    ...
    it('returns cached values when requesting the same chunk', async () => {
      const source = 'https://example.com/v2-chunks'
      const zmetadata = createV2Metadata()
  ...
  describe('v3', () => {
    ...
    it('reuses cached metadata', async () => {
      const source = 'https://example.com/v3-cached'
      const metadata = createV3Metadata()
      const arrayMetadata = createV3ArrayMetadata()
    ...
    it('returns cached values when requesting the same chunk', async () => {
      const source = 'https://example.com/v3-chunks'
      const metadata = createV3Metadata()
      const arrayMetadata = createV3ArrayMetadata()
      const chunkValues = [5, 6, 7, 8]
